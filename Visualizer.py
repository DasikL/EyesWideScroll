...
def filter_noise(
    ...
    if len(sacc_indices) > 0:
        ...
        for start, end in zip(seg_starts, seg_ends):
            ...
            amp = np.hypot(df.loc[end, 'x'] - df.loc[start, 'x'],
...
for filtered_df in filtered_df_list:
    adjusted_df = filtered_df[(filtered_df['x'] >= min_rel_x) & (filtered_df['x'] <= max_rel_x)].copy()
    adjusted_df['x_px'] = ((adjusted_df['x'] - min_rel_x) / (max_rel_x - min_rel_x)) * img_w
...
for screening in adjusted_df_list:
    ...
    heatmap_blurred = gaussian_filter(heatmap_data.T, sigma=25)  # .T to match image axes

    # 3. Normalize and mask
    heatmap_norm = heatmap_blurred / np.max(heatmap_blurred)
    heatmap_masked = np.ma.masked_where(heatmap_norm < 0.01, heatmap_norm)  # hide near-zero
    ...
    plt.savefig(folder+"/heatmap_overlay_"+image_name+".png", dpi=dpi, bbox_inches='tight', pad_inches=0)
    plt.close()
    fig = plt.figure(figsize=(img_w / dpi, img_h / dpi), dpi=dpi)
    ...
    plt.savefig(folder+"/heatmap_only_"+image_name+".png", dpi=dpi, bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()